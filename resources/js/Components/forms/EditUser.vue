...
<template>
    <form @submit.prevent="submit">
        ...
        <div class="font-bold d-block w-full mt-4">
            <span>Roles</span>
        </div>
        <div class="mb-4 w-full flex flex-wrap">
            <div class="w-1/2 my-2" v-for="(role, index) in roles" :key="index">
                <Checkbox v-model="form.roles" :input-id="'role' + index" name="role" :value="role.name" />
                <label :for="'role' + index" class="ml-2">{{ role.name }}</label>
            </div>
        </div>
